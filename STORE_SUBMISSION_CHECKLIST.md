# ChatRivet v1.3.3 リリース準備チェックリスト

このチェックリストは、公開済みv1.3.2の互換性修正版として、現在のmainを基に複数タブ間のタイトル同期・pin更新競合を防ぐ修正を含むv1.3.3を準備し、Chrome Web Storeへ提出するために使用します。Dashboard変更・審査送信は、ユーザーの最終承認後に行います。
...
- [x] `manifest.json`の`version`を`1.3.3`へ更新する。
...
- [x] 複数タブ間のタイトル同期・pin更新競合により、古いタイトルへの復帰や解除済みpinの復活が起きないことを自動テスト・実機確認する。
- [x] SPA同期、Project、安全側fallback、modifier・keyboard操作、Recent共存、cross-context競合を含む申請版の全72件回帰テストを行う。
...
- [x] `dist/ChatRivet-1.3.3.zip`を新規作成する。
...
- [x] ZIP内Manifestが有効で、versionが`1.3.3`であることを確認する。
...
- [ ] 問題がなければ申請版Commitへ注釈付きtag `v1.3.3`を作成する。
- [ ] release branchをPushし、安全にmainへ統合する。
- [ ] mainと`v1.3.3` tagをoriginへPushする。
- [ ] ローカルmain、origin/main、`v1.3.3`が意図したCommitを指すことを確認する。

## Dashboard最終監査・再提出

- [ ] 新ZIPをChrome Web Store Developer Dashboardへアップロードする。
- [ ] Dashboardに表示されるversionが`1.3.3`であることを確認する。
